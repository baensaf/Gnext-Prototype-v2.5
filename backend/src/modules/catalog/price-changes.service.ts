...
/** What a price change does: to base prices or one list, how much, from when. */
...
export class PriceChangeService implements OnApplicationBootstrap, OnModuleDestroy {
        ...
        const replaced = (
          await em.find(PriceEntry, {
            where: {
              ...
              effective_from: LessThanOrEqual(at),
            },
          })
        ).filter((e) => isLiveAt(e, at) && new Date(e.effective_from).getTime() < at.getTime());
   ...
   * (the product page, reports, import/export) says what the register charges. Marks the
   * changes that have started as applied. Every tenant when none is given.
   */
