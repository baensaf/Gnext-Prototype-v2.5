...
Primary navigation groups, in order: **Home**, **Sell** (POS, Dine-in, Orders), **Operations** (KDS, Delivery, Cashier, Payments/Refunds, Monitoring), **Customers**, **Catalog & Pricing**, **Discounts & Credit**, **Reports**, **Simulation Center**, **Audit**, **Settings**. The last selected branch is a header context filter for pages that support it; it never hides tenant-wide settings. A persistent **SIMULATED ENVIRONMENT** banner appears on simulator, kiosk hardware status, external payment, print, branch-status, and sync pages.
